...
{
    public partial class frmRicercaByOfferta : Form
    {
        private int IdLibroGlobale;

        public frmRicercaByOfferta()
        {
            InitializeComponent();
        }
        ...
        private void frmRicercaByOfferta_Load(object sender, EventArgs e)
        {
            ...
            // ----- Offerte (visualizza sulla dgv) -----
            cmbOfferte.DataSource = ListaOfferte.elencoOfferteLibri();
        ...
        private void visElencoLibri()
        {
            ...
            insLibro.Libro.IdOffLibro = Convert.ToInt32(cmbOfferte.SelectedValue);

            dgvLibri.DataSource = null;

            if(chkAnnullati.Checked) caricaLibri(insLibro.elencoLibriByOffertaAnnullati());
            else caricaLibri(insLibro.elencoLibriByOfferta());
        }
        ...
        private void cmbOfferte_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cmbOfferte.SelectedIndex != -1 && cmbOfferte.ValueMember != string.Empty)
            {
                ...
                clsLibriController insLibro = new clsLibriController();
                insLibro.Libro.IdOffLibro = Convert.ToInt32(cmbOfferte.SelectedValue);
                caricaLibri(insLibro.elencoLibriByOfferta());
